...
final readonly class SubmitPurchaseOrder
{
    ...
    {
        return $this->transition($context, $purchaseOrderId, $idempotencyKey, 'procurement.po.submit', PurchaseOrderStatus::Draft, [
     ...
     */
    private function transition(TenantRequestContext $context, string $purchaseOrderId, string $idempotencyKey, string $action, PurchaseOrderStatus $from, array $changes): PurchaseOrder
    {
        ...
        return DB::transaction(function () use ($context, $purchaseOrderId, $idempotencyKey, $action, $from, $changes, $requestHash): PurchaseOrder {
            $po = $this->poForUpdate($context, $purchaseOrderId);
            $record = $this->idempotency->findForUpdate($context->tenantId, $po->outlet_id, $context->userId, $action, $idempotencyKey);
            ...
            $po->forceFill($changes)->save();
            $this->idempotency->create($context->tenantId, $po->outlet_id, $context->userId, $action, $idempotencyKey, $requestHash, 'purchase_order', $po->id);
