...
final readonly class RecordWaste
{
    ...
    public function handle(TenantRequestContext $context, WasteInput $input, string $idempotencyKey): InventoryStockMovement
    {
        ...
        return DB::transaction(function () use ($context, $input, $idempotencyKey, $reason, $quantity, $requestHash): InventoryStockMovement {
            ...
            $this->audit->handle(
                ...
                outletId: $input->outletId,
                actorUserId: $context->userId,
                eventType: 'inventory_waste.recorded',
    ...
    private function consumeApprovalIfRequired(
        ...
        if (! $this->approvalRequired($quantity)) {
            return;
        }
        ...
        $this->approvals->handle(
            ...
            requestFingerprint: self::approvalFingerprint($input->outletId, $input->itemId, $quantity, $reason, $idempotencyKey),
        );
    }

    private function approvalRequired(string $quantity): bool
    {
        $threshold = config('inventory.approval.waste_quantity_threshold', '0.000');
        $threshold = is_string($threshold) ? $threshold : '0.000';

        return $this->quantity->toScaled($quantity) > $this->quantity->toScaled($threshold);
    }

    private function requestHash(WasteInput $input, string $quantity, string $reason): string
    {
        return hash('sha256', json_encode([
            'approval_id' => $input->approvalId,
            'currency' => mb_strtoupper(trim($input->currency)),
